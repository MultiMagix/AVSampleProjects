...
unsigned RTSPClient::sendAnnounceCommand(char const* sdpDescription, responseHandler* responseHandler, Authenticator* authenticator, const char* extraHeadersFromClient) {
  if (authenticator != NULL) fCurrentAuthenticator = *authenticator;
  envir() << "To add extra headers to send request : \n" << extraHeadersFromClient << "\n";
  return sendRequest(new RequestRecord(++fCSeq, "ANNOUNCE", responseHandler, NULL, NULL, False, 0.0, 0.0, 0.0, sdpDescription, extraHeadersFromClient), extraHeadersFromClient);
}
...
unsigned RTSPClient::sendRequest(RequestRecord* request, const char* extraHeadersFromClient) {
  if (fVerbosityLevel >= 1) {
	  envir() << "Sending request with custom header :"<< extraHeadersFromClient << "\n";

	   envir() << "request " << request->commandName() << "\n";
  }
	char* cmd = NULL;
  do 
  {
...
envir() << "enqueing the requesrt \n";
    ...
    //char* extraHeaders = (char*)"Video-payload: a=rtpmap:97 H264/90000|a=cliprect 0,0,144,176|a=framesize:97 176-144|a=fmtp:97 profile-level-id=42801E;sprop-parameter-sets=Z0KAHpWgsTE=,aM48gA==;packetization-mode=1";
   //  [java] Audio-payload: a=rtpmap:99 AMR/8000/1|a=fmtp:99 octet-align=1
   //  [java] "; 
    ...
    if (strcmp(request->commandName(), "DESCRIBE") == 0) {
      ...
      char* extraReqHeaders = request->GetExtraClientHeader();
	  char* strContentHeader = "Content-Type: application/sdp\r\n";
	  if(extraReqHeaders != NULL)
	  {
		  extraHeaders = new char [strlen(strContentHeader) + strlen(extraReqHeaders) + 1];
		  extraHeadersWereAllocated = True;
		  sprintf(extraHeaders, "%s%s", strContentHeader, extraReqHeaders);	 
	  }
	  else
	  {
		extraHeaders = strContentHeader;
	  }
	  if (fVerbosityLevel >= 1)
	  {
		   envir() << "Extra headers : \n " << extraHeaders <<"\n";
	...
	extraHeadersWereAllocated = True;
	 ...
	 envir() << " In send request constructing cmmd \n" ;
...
void RTSPClient::handleResponseBytes(int newBytesRead) {
  ...
  do {
    ...
    if (foundRequest != NULL) {
	 foundRequest->SetResponseHeader(fResponseBuffer);
  ...
  if (foundRequest != NULL && foundRequest->handler() != NULL)
  {
    ...
    if (responseSuccess) {
	  ...
	  if(foundRequest->GetExtraClientHeader() == NULL)
	  {
		(*foundRequest->handler())(this, resultCode, resultString);
	  }
	  else
	  {
		char* resultStringWithRespHeader = strdup(foundRequest->GetResponseHeader());
		(*foundRequest->handler())(this, resultCode, resultStringWithRespHeader);
	  }
    } 
	else {
...
RTSPClient::RequestRecord::RequestRecord(unsigned cseq, char const* commandName, responseHandler* handler,
					 ...
					 double start, double end, float scale, char const* contentStr, const char* extraHeaderFromClient)
  : fNext(NULL), fCSeq(cseq), fCommandName(commandName), fSession(session), fSubsession(subsession), fBooleanFlags(booleanFlags),
    fStart(start), fEnd(end), fScale(scale), fContentStr(strDup(contentStr)), fHandler(handler), fExtraHeadersFromClient(strdup(extraHeaderFromClient))
{
}
