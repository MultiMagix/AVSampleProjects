...
std::string input_file = "/home/phani/B-Frames.mp4";
...
int ffmpeg_output_initialization()
{
    ...
    output_codec_context_video->gop_size = 50; //input_codec_context_video->gop_size;

    output_codec_context_video->time_base = AVRational{1, input_fps};
    stream->time_base = input_codec_context_video->time_base;

    av_opt_set(output_codec_context_video->priv_data, "preset", "ultrafast", 0);
    av_opt_set(output_codec_context_video->priv_data, "tune", "zerolatency", 0);
    ...
    while (av_read_frame(input_format_context, &packet) >= 0) {
        ...
        if(input_format_context->streams[packet.stream_index]->codecpar->codec_type == AVMEDIA_TYPE_VIDEO)
        {
            ...
            while (response >= 0) {
                ...
                int frame_size = av_image_get_buffer_size(AV_PIX_FMT_YUV420P, iframe->width, iframe->height, 1);
                ...
                if (response >= 0) {
                  if (encode_video(iframe)) return -1;
                }
